...
public class StudentGradeDAOIpml extends DAO<StudentGrade> implements StudentGradeDAO {
	...
	public StudentGrade get(Long studentId, String courseName) {
		String sql = "select * from studentgrade where studentid = ? and courseName = ?";
		return get(sql, studentId, courseName);
	}
	...
	public void updateByStudentIdAndCourseName(StudentGrade studentGrade) {
		String sql = "update studentgrade set yearTerm = ?, studentName = ?,courseId = ?,  " +
				"score = ? ,gradePoint = ? ,credit = ? ,courseClass = ? ,studyWay = ?, examWay = ?, " +
				"gradeWay = ?, effectivity = ?, remarks = ? where studentId = ? and courseName = ?";
		update(sql ,studentGrade.getYearTerm(), studentGrade.getStudentName(), 
				studentGrade.getCourseId(),studentGrade.getScore(),studentGrade.getGradePoint(),
				studentGrade.getCredit(),studentGrade.getCourseClass(),studentGrade.getStudyWay(),studentGrade.getExamWay(),
				studentGrade.getGradeWay(),studentGrade.getEffectivity(),studentGrade.getRemarks(), studentGrade.getStudentId(),studentGrade.getCourseName());
